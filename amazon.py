...
def scrape():
    ...
    rating = item.i.text
    producturl ='https://www.amazon.in' + atag.get('href')
    try:
        delivery_parent = item.find(
            'div', class_="a-row s-align-children-center")
        delivery = delivery_parent.find(
            'span', class_='a-color-base puis-medium-weight-text').text
    ...
    result = (name, price, price_before, rating, delivery, producturl)
    print(result)
